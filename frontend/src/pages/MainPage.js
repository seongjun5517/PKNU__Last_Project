...
function MainPage() {
  ...
  const handleAddEntry = () => {
    ...
    if (repeatOn && repeatDays.length > 0) {
      ...
      setEntries((prev) => {
        const next = { ...prev };
        for (let i = 0; i < REPEAT_RANGE_DAYS; i++) {
          const d = new Date(start);
          d.setDate(start.getDate() + i);
          if (repeatDays.includes(d.getDay())) {
            const key = toDateKey(d);
            const newEntry = { ...baseEntry, id: `${repeatGroupId}-${key}`, repeatGroupId };
            next[key] = [...(next[key] || []), newEntry];
          }
        }
        return next;
      });
    } else {
      const newEntry = { ...baseEntry, id: Date.now() };
      setEntries((prev) => ({
        ...prev,
